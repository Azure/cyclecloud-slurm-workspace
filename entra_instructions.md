...
Replace the values for ```LOCATION```,```ENTRA_MI_RESOURCE_GROUP```, ```MI_NAME```, and ```APP_NAME``` in the below script, including ```<``` and ```>```.
...
az deployment group create -g $ENTRA_MI_RESOURCE_GROUP--template-uri https://raw.githubusercontent.com/Azurecyclecloud-slurm-workspace/refs/heads/abatallas/app_registrationbicep/entra/ccwEntraApp.json --parameters @app.json --name$ENTRA_DEPLOYMENT_NAME
